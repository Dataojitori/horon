...
编译器从左到右检查 `steps`，并累积路线中已经激活的节点：

- **→（有序）**：每个途经点都必须对应结果中一次具体的箭头到达，而且这些到达必须按输入顺序满足。
- **&（无序）**：`&` 的成员没有先后顺序。如果 `N = B & C`，那么箭头到达 `N` 时，`steps` 写 `B, C` 或 `C, B` 都可以。
- **允许插入中间节点**：要求 `A, B, C` 时，`A → X → B → Y → C` 可以通过。
- **允许跳过过早出现的点**：`A → C → B → C` 可以忽略第一个 C，用后一个 C 满足 `A, B, C`。

走过 `A → B` 时，B 和表示这条关系的概念会同时激活，因此后续既可以从 B 继续，也可以从关系概念继续。

...
- **`→`** 分隔位置（position）。左侧位置参与定义右侧位置，方向不可交换。
- **`&`** 在同一位置内并列成员。同位置内的成员可以交换顺序，语义不变。

两者在同一个表达式里协同工作。`→` 决定序列有几个位置，`&` 决定每个位置有几个成员。

```text
A → B           # pos1: A,  pos2: B
A → B → C       # pos1: A,  pos2: B,  pos3: C  （不可分割的三段序列）
o → p → E & R   # pos1: o,  pos2: p,  pos3: {E, R}
A & B            # pos1: {A, B}  （只有一个位置）
```

**不可分割性**：`A → B → C` **不等于**先建 `M = A → B` 再建 `M → C`。如何定义表达式决定了视角——`A → B → C` 中不存在独立的 `A → B` 概念。

**Inbound / Outbound 的统一规则**：inbound 和 outbound 永远只在相邻 position 之间产生。概念出现在某个位置，它的「下一个 position」是 outbound 方向，「上一个 position」是 inbound 方向。`&` 并列的成员同属一个 position，所以它们不会互相出现在对方的 inbound/outbound 里。只有一个位置的表达式（如 `A & B`）没有方向差，因此不产生 inbound/outbound 关系。

**`&` 不代表推导**：`&` 仅确认了共起——A 和 B 同时构成了某个概念，但绝不意味着从 A 能推导出 B。若需明确表达两个概念互为因果或完全等价，应建立两条有向边：`A → B` 与 `B → A`。
...
- **交集（重叠空间）**：当同一 position 内有多个 `&` 并列成员时，该位置的定义空间为这些成员的相交部分。
...
**`steps` 中的每个概念，都必须在最终 `compiled_route` 里真正用到。**除起点 `steps[0]` 外，编译器从左到右检查每个 step。每个 step 必须被结果中的某条箭头以下列两种方式之一使用：

- 箭头直接到达该 step。
- 箭头到达另一个概念，而那个概念的 `&` expression 中含有当前 step。该 expression 中的其他成员，要么在前面的路线中已经到达，要么是紧接在当前 step 后面、并由这条箭头一起满足的 steps。

**使用了 step 的分支必须继续汇入通往 goal 的路线。step 只在死胡同分支里出现，不算通过。**

例如，对于 `compile([Start, A, B], Goal)`，图中只需要有 `Start → N`、`N = A & B`、`N → Goal`。最终路线是 `Start → N → Goal`。

- **只给起点和终点**：不加任何限定，系统在全图搜索，返回找到的最优路线。
- **加入途经点**：结果必须按输入顺序用到这些点，中间可以夹其他节点；如果箭头到达的概念的 `&` expression 中含有多个相邻途经点，这条箭头可以一次用到它们。**途经点的顺序就是计划的顺序**。
...
编译器从左到右检查 `steps`。`steps` 不是一组必须彼此直接以 `→` 相连的边，也不要求它们在结果中紧挨着出现；它只约束最终 `concept_order` 中这些 occurrence 的相对顺序。换句话说，`steps` 必须按给定顺序构成结果序列的一个子序列，中间可以穿插任意由路线带入的概念。

- **`concept_order`**：编译返回的实际经过节点序列（包含中继节点、成员与关系概念自身）。例如 `R = A → B → A` 展开为 `[A, B, A, R]`（重复经过不去重）；`N = B & C` 展开为 `[B, C, N]` 或 `[C, B, N]`。
- **允许插入任意中间节点**：要求 `A, B, C` 时，只要结果中的 occurrence 顺序是 `A, ..., B, ..., C` 就可以通过。例如 `A → X → B → Y → C` 合法；表达式概念、`&` 分支成员及其他中继节点也都可以出现在省略号位置。
- **允许跳过过早出现的点**：`A → C → B → C` 可以忽略第一个 C，用后一个 C 满足 `A, B, C`。
...
- **compiled_route**：整条因果路线（含系统补全的中继站和 `&` 的成员分支）。
...
1. **断口（break）**：按给定顺序检查时，最先无法满足的两个相邻要求。
2. **compiled_route**：从起点到断口左端已经验证过的路线；如果第一段就断，才是空列表。
3. **自由通路（detour）**：从起点或断口前已经确立的任一途经点出发，寻找一条到终点的路。通 → 把这段路线附上；不通 → detour 为 null。关系概念可以作为途中节点，但不会成为返回路线的起点。
...
判断规则：
- `→` 分隔位置：有多个 position → 表达式含 `→`。position 1 = 链首（定义者方向），最高 position = 链尾（被定义者方向）
- `&` 并列成员：同一 position 内多个成员即为 `&` 并列（如 `A → B → C & D` 中 C、D 同属 position 3）
- 只有 position 1 → 表达式只含 `&`，无方向（如 `A & B`）
- 没有成员 → 尚未拆解
- 同一 concept 下不允许两个 variation 拥有完全相同的 compose_members（应用层校验）
- 编译器把整个 variation 作为不可分割的因果动作：先满足 position 1，再依次经过后续 position，最后激活承载该 variation 的概念。不会把多段表达式伪拆成若干独立二元关系。
...
### 编译
- `compile(steps, goal)` — 行动路径检测器。从左到右检查途经点是否被最终路线实际使用。详见[推理 = 编译](#推理--编译)

...
- 箭头直接到达该 step。
- 该 step 是某个 AND/OR 节点的成员。如果是 AND 节点，要将其走通，其所有成员必须被合法“凑齐”（要么在之前的合法路径中已经被走到，要么刚好在用户 `steps` 列表中紧跟其后被打包满足）；如果是 OR 节点，只要到达了该 step 这一单个成员，就已经合法激活了该 OR 节点。但无论哪种情况，走通的这个容器节点（AND/OR）**必须在后续的路径中对到达目标（goal）做出了实际贡献**，否则这条分支就会被视为死胡同，其中的 step 也不算真正被“用到”。
...
- **加入途经点**：结果必须按输入顺序用到这些点，中间可以夹其他节点；如果箭头到达的概念的 AND/OR 集合中含有多个相邻途经点，这条箭头可以一次用到它们。**途经点的顺序就是计划的顺序**。
...
- **negated 是墙**：算法遇到 `status = negated` 的连线直接绕路退回。没有复杂的全局主动抑制或扩散逻辑。
...
- **允许插入任意中间节点**：要求途经 `A, B, C` 时，只要结果中的 occurrence 顺序是 `A, ..., B, ..., C` 就可以通过。例如 `A → X → B → Y → C` 合法。中间的省略号不仅可以是普通的中继节点，也可以是你为了走通某个 AND 节点而顺路集齐的其他子要素。但注意：**所有被卷进来的节点都必须由给定的起点/途经点合法走到，绝不能凭空出现**。只要路径合法且保证 `A`, `B`, `C` 在时序上是递进的，穿插多少图谱约束带来的副产物都是允许的。

...
def try_normalized_patch(
    ...
    """
    # Tradeoff: NFC the whole field so pos_map indexes one canonical string.
    # Result is NFC throughout, so a patch may recompose decomposed Unicode
    # outside the match (e.g. NFD text). Accepted; preserving raw bytes would
    # need a second map back to pre-NFC offsets.
